...
function WatercraftSchedulerUI() {
  ...
  var memberObject;
  const url =
    "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/getschedule/" +
  ...
  const getReservations = () => {
    axios.get(url).then((res) => {
      ...
      for (let i = 0; i < res.data.length; i++) {
        const memberColor =
          "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/member/getMember/" +
          res.data[i].userId;
        axios.get(memberColor).then((result) => {
          color = result.data.schedulercolor;
          setEventsCalendar(
            res.data[i].reservation,
            res.data[i].scheduleId,
            color
          );
        });
      }
  ...
  const setEventsCalendar = (events, scheduleId, color) => {
    for (let i = 0; i < events.length; i++) {
      ...
      event.allDay = true;
      event.title =
        String(events[i].startHour) + " - " + String(events[i].endHour);
      event.id = scheduleId;
      event.color = color;
  ...
  const urlMembers =
    "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/member/getMemberByWatercraft/" +
    universalWatercraftId;
  const urlLoggedInMember =
    "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/member/getMember/" +
  ...
  const monthSlots =
    "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/getslots/" +
    sessionStorage.getItem("userId");
    ...
    allReservations.map((eachReservation) => {
      ...
      if (parseInt(eachReservation.scheduleId) === parseInt(e.event.id)) {
        setDisplayEvent({
          ...
          toDate: eachReservation.reservation[
            eachReservation.reservation.length - 1
          ].forDate.split("T")[0],
          toHour:
            eachReservation.reservation[eachReservation.reservation.length - 1]
              .endHour,
  ...
  const handleDeleteEvent = (e) => {
    const deleteURL =
      "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/deleteschedule/" +
      displayEvent.scheduleId;
    axios.delete(deleteURL).then(
      (res) => {},
      (error) => {
        alert(
          "Unable to cancel reservation at this moment. Please contact the administrator"
        );
      }
    );
  ...
  const confirmReservation = () => {
    ...
    const endpoint =
      "http://ec2-18-237-18-199.us-west-2.compute.amazonaws.com:8080/addschedule";
    axios.post(endpoint, reservationObject).then(
      (res) => {
        //window.location.reload();
      },
      (error) => {
  ...
  return (
    <div>
          ...
          <div>
            <Grid container alignItems="center" justify="center">
              <Grid item sm={3} align="center">
                <h3>Holiday Slots Remaining:</h3>
                {displaySlot.holidaySlot}
              </Grid>
              <Grid item sm={3} align="center">
                <h3>Previous Month's Slots:</h3>
                {displaySlot.prevMonthSlot}
              </Grid>
              <Grid item sm={3} align="center">
                <h3>Current Month's Slots:</h3>
                {displaySlot.currMonthSlot}
              </Grid>
              <Grid item sm={3} align="center">
                <h3>Next Month's Slots:</h3>
                {displaySlot.nextMonthSlot}
              </Grid>
            </Grid>
          </div>
          <hr
      ...
      >
        ...
        {(sessionStorage.getItem("role") == "Admin" ||
          displayEvent.userId == sessionStorage.getItem("userId")) && (
          <DialogActions>
      ...
      <Dialog
        open={open}
        onClose={handleDialogCloseOnCancel}
        aria-labelledby="form-dialog-title"
        fullWidth
        maxWidth="sm"
      >
              ...
              <FormControl variant="outlined" fullWidth>
                ...
                >
                  {slotDropDown.map((slot, index) => {
                    return (
                      <MenuItem key={index} value={slot.slotString}>
                        {slot.slotString}
                      </MenuItem>
                    );
                  })}
              ...
              <Grid container>
                ...
                <Grid item sm={12} fullWidth>
                  <FormControlLabel
                    control={
                      <Checkbox
                        onChange={handleCheckboxConcierge}
                        name="conciergeRequired"
                        checked={values.checkedConcierge}
                      />
                    }
                ...
                <Grid item sm={12} fullWidth>
                  <FormControlLabel
                    control={
                      <Checkbox
                        onChange={handleCheckboxCrew}
                        name="crewRequired"
                        checked={values.checkedCrew}
                      />
                    }
                    label="Request Crew"
                  />
                  ...
                  <>
                    <Grid item sm={12} fullWidth>
                      <FormControlLabel
                        control={
                          <Checkbox
                            onChange={handleCheckboxQuota}
                            name="notFromUserQuota"
                            checked={values.notFromUserQuota}
                          />
                        }
                    ...
                    <Grid item sm={12} fullWidth>
                      <FormControlLabel
                        control={
                          <Checkbox
                            onChange={handleCheckboxMaintenence}
                            name="forMaintenence"
                            checked={values.forMaintenence}
                          />
                        }
        ...
        <DialogActions>
          <Button
            variant="contained"
            onClick={handleDialogCloseOnCancel}
            color="secondary"
          >
            Cancel
          </Button>
          <Button
            variant="contained"
            onClick={handleDialogCloseOnConfirm}
            color="primary"
          >
            Confirm
          </Button>
